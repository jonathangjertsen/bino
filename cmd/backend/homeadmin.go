...
func (server *Server) getHomesHandler(w http.ResponseWriter, r *http.Request, commonData *views.CommonData) {
	ctx := r.Context()
	...
	homes := make([]views.Home, len(homesDB))
	...
	for _, user := range usersDB {
		...
		if user.HomeID.Valid {
			for i, home := range homesDB {
				if home.ID == user.HomeID.Int32 {
					homes[i].Users = append(homes[i].Users, views.User{
						ID:          user.ID,
		...
		if !found {
			homeless = append(homeless, views.User{
				ID:          user.ID,
				DisplayName: user.DisplayName,
				Email:       user.Email,
			})
	...
	_ = views.HomesPage(commonData, homes, homeless).Render(ctx, w)
}

func (server *Server) postHomeHandler(w http.ResponseWriter, r *http.Request, commonData *views.CommonData) {
...
func (server *Server) postHomeCreateHome(w http.ResponseWriter, r *http.Request, commonData *views.CommonData) {
	ctx := r.Context()
...
func (server *Server) postHomeAddUser(w http.ResponseWriter, r *http.Request, commonData *views.CommonData) {
	ctx := r.Context()
